            ...
            switch ((String) request[0]){
                ...
                case "queryAccount":
                    try{
                        ...
                        Account account = Datasource.getInstance().queryAccount(id);
                        output.writeObject(account);
                    } catch (IOException e){
                        System.out.println("Error sending accounts to client: " + e.getMessage());
                    } break;
                case "queryTransactionsForTable":
                    try{
                ...
                case "queryTransaction":
                    try{
                        ...
                        System.out.println("Error sending transaction info to client: " + e.getMessage());
                    } break;
                ...
                case "queryDetails":
                    try{
                        int id = (Integer) request[1];
                        Details details = Datasource.getInstance().queryDetails(id);
                        output.writeObject(details);
                    } catch (Exception e){
                        System.out.println("Error sending details: " + e.getMessage());
                    } break;
                case "saveDetails":
                    try{
                        Details details = (Details) request[1];
                        int id = (Integer) request[2];
                        if(Datasource.getInstance().saveDetails(details, id)){
                            output.writeObject(confirmation);
                        } else {
                            output.writeObject(error);
                        }
                    } catch (IOException e){
                        System.out.println("Error sending confirmation of details to client: " + e.getMessage());
                    } break;
                case "queryAllAccounts":
                    try{
                        ...
                        System.out.println("Error sending list of accounts to client: " + e.getMessage());
                    } break;
                default:
